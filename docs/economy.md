...
Convex models vault-era in-game EARTH with `earth_ledgerAccounts`, `earth_ledgerEntries`, `earth_withdrawals`, and `earth_escrowReconciliations`. Raw Token-2022 units are stored as strings. `earth_characters.earth` remains a display/cache field updated from the ledger for existing UI compatibility, not canonical custody. Withdrawal requests move ledger balance from available to pending before server/on-chain authorization; completion removes pending after the Earth Vault release transaction, while expiry returns pending funds to available. Reconciliation reports compare escrow balance against `availableRaw + pendingWithdrawalRaw` liabilities.
