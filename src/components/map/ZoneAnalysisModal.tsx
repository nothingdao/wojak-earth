...
export default function ZoneAnalysisModal({
  ...
  // Supabase setup for direct database operations
  const supabase = createClient(
    'https://jnqmbveckrymyyoddsuw.supabase.co',
    'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InN1ZHVmbW1rZnVhd29tdmxya2hhIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0ODY3Nzk1OCwiZXhwIjoyMDY0MjUzOTU4fQ.nHcuupwtQEpnpgkrf1PqAiGaq0uyFQ9fly5uPRY1644'
  )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editedLocation.id && editedLocation.id !== location.id) {
        ...
        try {
          console.log('Step 1: Update child locations parent_location_id references')
          if (childLocations.length > 0) {
            const { error: childError } = await supabase
              .from('locations')
              .update({ parent_location_id: editedLocation.id })
              .eq('parent_location_id', location.id)

            if (childError) throw childError
            console.log(`✅ Updated ${childLocations.length} child locations`)
          }

          console.log('Step 2: Update character current_location_id references')
          const { error: charError } = await supabase
            .from('characters')
            .update({ current_location_id: editedLocation.id })
            .eq('current_location_id', location.id)

          if (charError) console.warn('Characters update failed:', charError)

          console.log('Step 3: Create new location with new ID')
          const newLocationData = { ...location, ...editedLocation }
          ...
          console.log('Step 4: Delete old location')
          ...
          alert(`Successfully migrated location ID from "${location.id}" to "${editedLocation.id}"`)
          ...
          alert(`Migration failed: ${migrationError.message}. Please use database tools for manual migration.`)
        }
