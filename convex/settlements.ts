...
import {
  ...
  sessionIdToBytes,
} from "../packages/shared/vault/messages";
...
function findPlayerEmissionPda(player: PublicKey) {
...
function findGameSettlementPda(sessionId: Uint8Array) {
  return PublicKey.findProgramAddressSync(
...
export const submitAstrdsSettlement = internalAction({
  ...
  handler: async (ctx, args): Promise<{ submitted: boolean; signature?: string }> => {
