...
function parseDepositPool(pubkey, data) {
    return {
...
async function getTokenMetadata(mintAddress, rpcEndpoint) {
    try {
...
export const vaultHealthCheck = action({
    ...
    handler: async (ctx) => {
        ...
        const onChainPools = accounts.map(({ pubkey, account }) => parseDepositPool(pubkey, Buffer.from(account.data)));
        const convexDeposits = await ctx.runQuery(internal.spaceDeposits.getAllDeposits, {});
        const convexByKey = new Map(convexDeposits.map((d) => [`${d.walletAddress}:${d.mintAddress}`, d]));
        const onChainByKey = new Map(onChainPools.map((p) => [`${p.depositor}:${p.mint}`, p]));
        const missing = [];
        const mismatched = [];
        const healthy = [];
    ...
    handler: async (ctx, { poolAddresses }) => {
        ...
        for (const address of poolAddresses) {
            try {
                ...
                const pool = parseDepositPool(pubkey, Buffer.from(accountInfo.data));
