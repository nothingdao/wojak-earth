...
import { createInitializeMintInstruction, createInitializeMetadataPointerInstruction, createAssociatedTokenAccountIdempotentInstruction, createMintToInstruction, tokenMetadataInitializeWithRentTransfer, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, getMintLen, ExtensionType, getAssociatedTokenAddressSync, } from "@solana/spl-token";
...
const deriveMintKeypair = (authority, tokenDir) => {
...
export const mintTestToken = action({
    ...
    handler: async (_ctx, { playerPublicKey, amount, decimals = 6, tokenDir, tokenName, tokenSymbol }) => {
        ...
        const playerAta = getAssociatedTokenAddressSync(mintPubkey, playerPubkey, false, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
        ...
        if (!mintAccountInfo) {
            ...
            }), createInitializeMetadataPointerInstruction(mintPubkey, authority.publicKey, mintPubkey, TOKEN_2022_PROGRAM_ID), createInitializeMintInstruction(mintPubkey, decimals, authority.publicKey, null, TOKEN_2022_PROGRAM_ID), createAssociatedTokenAccountIdempotentInstruction(authority.publicKey, playerAta, playerPubkey, mintPubkey, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID), createMintToInstruction(mintPubkey, playerAta, authority.publicKey, BigInt(amount) * BigInt(10 ** decimals), [], TOKEN_2022_PROGRAM_ID));
            ...
            await tokenMetadataInitializeWithRentTransfer(connection, authority, mintPubkey, authority.publicKey, authority, tokenName, tokenSymbol, metadataUri, [], { commitment: "confirmed" }, TOKEN_2022_PROGRAM_ID);
            return {
                success: true,
                signature: sig1,
        ...
        const tx = new Transaction().add(createAssociatedTokenAccountIdempotentInstruction(authority.publicKey, playerAta, playerPubkey, mintPubkey, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID), createMintToInstruction(mintPubkey, playerAta, authority.publicKey, BigInt(amount) * BigInt(10 ** decimals), [], TOKEN_2022_PROGRAM_ID));
