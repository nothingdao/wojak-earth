...
export const mineAtLocation = mutation({
    ...
    handler: async (ctx, { walletAddress, locationId }) => {
        ...
        if (resources.length === 0)
            throw new Error("No resources at this location");
        ...
        const existing = await ctx.db
            ...
            .withIndex("by_character_item", (q) => q.eq("characterId", character._id.toString()).eq("itemId", resource.itemId))
            .unique();
        if (existing) {
...
export const getGlobalActivity = query({
    ...
    handler: async (ctx, { limit = 20 }) => {
        const txns = await ctx.db
            ...
            .order("desc")
            .take(limit);
        return Promise.all(txns.map(async (txn) => {
            const character = await ctx.db
                .query("earth_characters")
