...
export const getWithChapters = query({
    ...
    handler: async (ctx, { storyId }) => {
        ...
        const chapters = await ctx.db
            ...
            .collect();
        const chaptersWithEvents = await Promise.all(chapters.map(async (chapter) => {
            const events = await ctx.db
                .query("earth_events")
                .withIndex("by_chapter", (q) => q.eq("chapterId", chapter._id.toString()))
                .collect();
            const eventsWithChoices = await Promise.all(events.map(async (event) => {
                const choices = await ctx.db
                    .query("earth_choices")
                    .withIndex("by_event", (q) => q.eq("eventId", event._id.toString()))
                    .collect();
                const choicesWithConsequences = await Promise.all(choices.map(async (choice) => {
...
export const getChapterWithContent = query({
    ...
    handler: async (ctx, { chapterId }) => {
        ...
        const eventsWithChoices = await Promise.all(sortedEvents.map(async (event) => {
            const choices = await ctx.db
                .query("earth_choices")
                .withIndex("by_event", (q) => q.eq("eventId", event._id.toString()))
                .collect();
            const sortedChoices = [...choices].sort((a, b) => a.orderIndex - b.orderIndex);
            const choicesWithConsequences = await Promise.all(sortedChoices.map(async (choice) => {
                const consequences = await ctx.db
                    .query("earth_consequences")
                    .withIndex("by_choice", (q) => q.eq("choiceId", choice._id.toString()))
                    .collect();
