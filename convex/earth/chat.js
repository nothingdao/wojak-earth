...
export const getByLocation = query({
    ...
    handler: async (ctx, { locationId, limit = 50 }) => {
        const messages = await ctx.db
            ...
            .order("desc")
            .take(limit);
        const withCharacters = await Promise.all(messages.map(async (msg) => {
...
export const send = mutation({
    args: {
        ...
        locationId: v.string(),
        message: v.string(),
        messageType: v.optional(v.union(v.literal("CHAT"), v.literal("EMOTE"), v.literal("SYSTEM"), v.literal("WHISPER"))),
    },
