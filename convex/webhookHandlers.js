...
export const handleTreasuryWebhook = httpAction(async (ctx, request) => {
    ...
    for (const tx of body) {
        ...
        const claim = await ctx.runQuery(internal.spaceDeposits.getClaimBySignature, { txSignature: tx.signature });
        if (claim)
            continue;
        ...
        const pendingDeposit = await ctx.runQuery(internal.spaceDeposits.getDepositBySignature, { txSignature: tx.signature });
